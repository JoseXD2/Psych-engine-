...
import sys.io.File;  //funker
import sys.FileSystem; //funker #2
...
class TitleState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
                Lib.current.loaderInfo.uncaughtErrorEvents.addEventListener(UncaughtErrorEvent.UNCAUGHT_ERROR, onTxt);
    ...
    function onTxt(e:UncaughtErrorEvent):Void
    {
        var errMsg:String = "";
        var path:String;
        ...
        dateNow = dateNow.replace("nao sei oque colocar lol", "_");
        dateNow = dateNow.replace(":", "'");

        path = "./fucklol/" + "um_txt_wow" + dateNow + ".txt";

        errMsg += "\nUma pessoa invade o seu cell: " + e.error + "\nmario games";

        if (!FileSystem.exists("./fucklol/"))
            FileSystem.createDirectory("./fucklol/");

        File.saveContent(path, errMsg + "\n");

        Sys.println(errMsg);
        Sys.println("txt salvolol " + Path.normalize(path));

        Application.current.window.alert(errMsg, "txt salvo");
        DiscordClient.shutdown();
        Sys.exit(1);
    }
    #end
